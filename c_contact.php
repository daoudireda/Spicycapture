...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = stripslashes(trim($_POST['contact-name']));
    $email = stripslashes(trim($_POST['contact-email']));
    $subject = stripslashes(trim($_POST['contact-subject']));
    $message = stripslashes(trim($_POST['contact-project']));
 ...
 /*   $response = stripslashes(trim($_POST['g-recaptcha-response']));*/
   ...
   /* if ($decode['success'] == true) {*/
        ...
        if (empty($name)) {
            $errors['contact-name'] = 'Le nom est obligatoire.';
        }

       

        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $errors['contact-email'] = 'L\'email est obligatoire.';
        }

        if (empty($subject)) {
            $errors['contact-subject'] = 'Le sujet est obligatoire.';
        }

        if (empty($message)) {
            $errors['contact-project'] = 'Le message est obligatoire.';
        }
